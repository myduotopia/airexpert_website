...
- 同一列若兩張卡的內容都有（例：專用油寫「例」、過濾系統欄寫「乾燥機12"散熱馬達+葉片」）
  → 拆成「兩列」輸出，各自只保留屬於自己那張卡的欄位值，日期 / 維護員 兩列都填，
    再各自標上 belongs_to。
- card_kind = "compressor" 時**也要照上面的規則逐列判斷**，不可一律標 "compressor"：
  表頭沒寫過濾型號，不代表這張紙上沒有第二台機器 —— 舊卡很常見「表頭只有機號，
  但列中寫著 AD480×1、乾燥機12"散熱馬達+葉片、乾修:12"馬達+葉片」。這種列一樣標
  belongs_to = "filter"（filter_spec 仍維持 ""，它只能來自表頭）。
