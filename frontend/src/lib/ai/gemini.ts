...
【機號欄】機號常常不是原廠序號 —— 現場很多卡寫的是「A機」「1號機」「B」這類代稱；
過濾系統卡更常在機號位置寫過濾器型號（如「過濾 AL 010N + LM-P-010」）。
- 一律「照卡片原文」填入 serial_no，**不要**自行補上客戶名稱或任何前綴，也不要改寫成序號樣式。
- 卡上完全找不到機號時 serial_no 填 ""，不要拿機型或客戶編號充數。
